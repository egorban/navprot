...
func TestNDTP_Parse(t *testing.T) {
	tests := []struct {
		...
		{"incorrectCS", ndtpIncorrectCS(), nil, new(NDTP), true},
		{"shortPacket", ndtpShort(), nil, new(NDTP), true},
		{"shortVeryPacket", ndtpVeryShort(), nil, new(NDTP), true},
		{"signatureNotFound", ndtpWithoutSignature(), nil, new(NDTP), true},
	}
...
func ndtpExtResult() *NDTP {
	data := ExtDevice{MesID: 1}
	nph := NphData{NphSrvExternalDevice, nphSedDeviceResult, false, 263, &data}
	npl := NplData{DataType: 0x02, PeerAddress: []byte{0, 4, 0, 0}, ReqID: 263}
	return &NDTP{&npl, &nph, packetExtResult()}
}
